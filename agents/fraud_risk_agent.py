...
class FraudRiskAgent(BaseAgent):
    ...
    def reason(self, context: Dict[str, Any]) -> Decision:
        ...
        # Velocity flag: for each transaction, does its customer have
        # >=VELOCITY_THRESHOLD transactions within VELOCITY_WINDOW_SECONDS
        # of it (symmetric window)? Computed once per customer via a
        # sorted-timestamp two-pointer sweep (left and right pointers
        # each advance monotonically at most len(customer's txns) times)
        # instead of re-sorting and re-scanning the customer's full
        # transaction list for every one of their transactions -- O(c log c)
        # overall (dominated by the per-customer sorts) instead of O(c^2)
        # in the worst case of one customer holding most transactions.
        velocity_flag_by_id: Dict[Any, float] = {}
        for cust_txns in by_customer.values():
            cust_sorted = sorted(cust_txns, key=lambda x: x["timestamp"])
            timestamps = [x["timestamp"] for x in cust_sorted]
            n_cust = len(timestamps)
            left = right = 0
            for i in range(n_cust):
                while timestamps[left] < timestamps[i] - VELOCITY_WINDOW_SECONDS:
                    left += 1
                if right < i:
                    right = i
                while right + 1 < n_cust and timestamps[right + 1] <= timestamps[i] + VELOCITY_WINDOW_SECONDS:
                    right += 1
                window_count = right - left + 1
                velocity_flag_by_id[cust_sorted[i]["transaction_id"]] = 1.0 if window_count >= VELOCITY_THRESHOLD else 0.0

        scored = []
        for t, z in zip(transactions, z_scores):
            velocity_flag = velocity_flag_by_id[t["transaction_id"]]
