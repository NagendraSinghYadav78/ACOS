...
Extends the 7-node scheduling comparison in langgraph_equivalent_dag.py
to a larger, structurally comparable DAG (50 nodes, 5 layers of 10
parallel nodes each feeding into the next layer), built in both ACOS's
Planner and LangGraph's StateGraph, measuring scheduling-only overhead
(no-op nodes) at this larger size. Scope: scheduling only, not a full
scaling study across multiple orders of magnitude, and no-op nodes are
not the same workload as real per-agent computation (Section 9 of the
paper).
"""
